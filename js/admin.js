...
function calculateBilling(){
  ...
  valid.forEach(o=>{
    ...
    const city=destinationCity(o.delivery_address);
    const key=`${price.toFixed(2)}|${city.toLocaleLowerCase('it-IT')}`;
    if(!groups.has(key))groups.set(key,{price,city,count:0,subtotal:0});
    const row=groups.get(key);
    row.count+=1;
    row.subtotal=Number((row.subtotal+price).toFixed(2));
  });
  const breakdown=[...groups.values()].sort((a,b)=>a.price-b.price||a.city.localeCompare(b.city,'it'));
  ...
  el('billingBreakdown').innerHTML=breakdown.length?breakdown.map(row=>`<div class="billingRow"><div><strong>${row.count} ${row.count===1?'ordine':'ordini'} · ${esc(row.city)}</strong><span>× ${esc(euro(row.price))}</span></div><strong>${esc(euro(row.subtotal))}</strong></div>`).join(''):'<p class="muted">Nessun ordine contabilizzabile nel periodo selezionato.</p>';
}


function destinationCity(address){
  const raw=String(address||'').trim();
  if(!raw)return 'Città non disponibile';
  const parts=raw.split(',').map(part=>part.trim()).filter(Boolean);
  const capIndex=parts.findIndex(part=>/^\d{5}$/.test(part));
  if(capIndex>=0&&parts[capIndex+1])return parts[capIndex+1];
  if(parts.length>=2){
    const last=parts[parts.length-1].replace(/\b(?:MI|MB|BG|BS|CO|CR|LC|LO|MN|PV|SO|VA)\b/gi,'').trim();
    if(last&&!/^\d{5}$/.test(last))return last;
  }
  const capMatch=raw.match(/\b\d{5}\s+([^,]+)/);
  if(capMatch&&capMatch[1])return capMatch[1].trim();
...
function downloadBillingExcel(){
  ...
  wsSummary['!cols']=[{wch:31},{wch:28},{wch:20},{wch:20}];
  wsDetail['!cols']=[{wch:12},{wch:20},{wch:22},{wch:34},{wch:34},{wch:20},{wch:22},{wch:18},{wch:22},{wch:18},{wch:22},{wch:13},{wch:20},{wch:30},{wch:20},{wch:20}];
  wsCancelled['!cols']=[{wch:12},{wch:20},{wch:22},{wch:34},{wch:34},{wch:20},{wch:13},{wch:14}];
  if(wsSummary['B9'])wsSummary['B9'].z='#,##0.00 [$€-it-IT]';
  r.breakdown.forEach((_,i)=>{
    const row=13+i;
    if(wsSummary[`C${row}`])wsSummary[`C${row}`].z='#,##0.00 [$€-it-IT]';
    if(wsSummary[`D${row}`])wsSummary[`D${row}`].z='#,##0.00 [$€-it-IT]';
  });
  for(let i=2;i<=detailRows.length;i++)if(wsDetail[`L${i}`])wsDetail[`L${i}`].z='#,##0.00 [$€-it-IT]';
  for(let i=2;i<=cancelledRows.length;i++)if(wsCancelled[`G${i}`])wsCancelled[`G${i}`].z='#,##0.00 [$€-it-IT]';
